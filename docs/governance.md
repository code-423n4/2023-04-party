...
After a crowdfund has been won, it creates a new governance `Party`. Contributors are minted NFT memberships in the new Party, containing voting power proportional to their contribution during the crowdfund. Voting power can be used to vote on proposals, which contain possible actions for the Party to execute.
...
1. `enableAddAuthorityProposal`: This flag determines whether the Party can propose and add new authorities using the `AddAuthorityProposal`. If set to `true`, the Party can add new authorities through proposals. If set to `false`, the Party cannot add new authorities.
