...
These contracts allow people to create and join a crowdfund, pooling ETH together to achieve a goal. Often this is to acquire an NFT, but for some crowdfunds (e.g. with `InitialETHCrowdfund` and `ReraiseETHCrowdfund`) the goal within itself is to raise ETH. For crowdfunds with the goal of acquiring NFTs, multiple crowdfund contracts exist for specific acquisition patterns.
...
- `uint40 duration`: How long a crowdfund has before the crowdfund expires.
